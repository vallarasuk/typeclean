 ...
 */
export function countTokens(text: string, model: 'openai' | 'claude' | 'gemini' = 'openai'): number {
  const chars = text.length;
  // Very rough approximations for fallback without heavy tokenizers
  switch (model) {
    case 'gemini': return Math.ceil(chars / 3.5);
    case 'claude': return Math.ceil(chars / 3.8);
    case 'openai':
    default:
